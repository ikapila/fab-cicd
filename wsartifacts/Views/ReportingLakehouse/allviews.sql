...
contract_id,
number,
'Receipt' AS receivable_type,
received_date AS _date,
(balance_amount * -1) AS balance_with_tax,
entity_type
FROM aggregated_odessa_dim_receipt AS r
WHERE r.current_flag = 1
AND r.status = 'Posted'
AND r.balance_amount != 0
AND r.received_date < CAST(GETDATE() as date)

),
invoices AS (
SELECT 
customer.[bk_aggregated_dim_customer_id] AS customer_id,
CASE WHEN drid.entity_type = 'CT' THEN drid.entity_id ELSE '' END AS contract_id,
dri.number,
drid.entity_type,
...
is_tax_deferral_applicable,
flfd.[calculated_tax_deferral_amount_amount],
cu.bk_aggregated_dim_customer_id AS customer_id,
c.status,
...
invoices_and_receipts AS (
SELECT 
invoices.customer_id,
invoices.contract_id,
invoices.number,
invoices.receivable_type,
invoices._date, 
...
SELECT 
COALESCE(receipts.customer_id, lf.customer_id) AS customer_id,
receipts.contract_id,
receipts.number,
receipts.receivable_type,
receipts._date,
receipts.balance_with_tax,
receipts.entity_type
FROM 
receipts
LEFT JOIN contract ON receipts.contract_id = contract.bk_aggregated_dim_contract_id
LEFT JOIN lf ON contract.bk_aggregated_dim_contract_id = lf.contract_id
...
_date,
balance_with_tax,
DATEDIFF(DAY, iar._date, GETDATE()) AS date_diff,
CASE WHEN iar._date >= CAST(GETDATE() as date) THEN 'Current'
 WHEN iar._date >= DATEADD(DAY, -30, CAST(GETDATE() as date)) THEN '1-30 Days'
  WHEN iar._date >= DATEADD(DAY, -60, CAST(GETDATE() as date)) THEN '31-60 Days'
    WHEN iar._date >= DATEADD(DAY, -90, CAST(GETDATE() as date)) THEN '61-90 Days'
        WHEN iar._date >= DATEADD(DAY, -120, CAST(GETDATE() as date)) THEN '91-120 Days'
        ELSE '120+ Days'
 END as age_bracket,

am.account_manager,
sr.primary_sales_rep
FROM invoices_and_receipts AS iar 
LEFT JOIN  aggregated_odessa_dim_customer AS customer ON iar.[customer_id] = customer.bk_aggregated_dim_customer_id AND customer.current_flag = 1 
LEFT JOIN aggregated_odessa_dim_contract AS c ON iar.contract_id = c.bk_aggregated_dim_contract_id AND c.current_flag = 1 --AND COALESCE(iar.entity_type, '') != 'Customer'
...
LEFT JOIN account_manager AS am ON customer.bk_aggregated_dim_customer_id  = am.party_id 
LEFT JOIN sales_rep AS sr ON customer.bk_aggregated_dim_customer_id  = sr.party_id 
...
sundries.source_system
 FROM flat_budgets_inc_history AS sundries
 JOIN flat_contracts_inc_history AS contracts ON sundries.contract_id = contracts.id
 JOIN flat_assets_inc_history AS assets ON sundries.asset_id = assets.id
...
LEFT JOIN flat_assets_inc_history AS assets ON sundries.asset_id = assets.id
LEFT JOIN flat_contracts_inc_history AS contracts ON sundries.contract_id = contracts.id
...
addDetails.tyre_configuration,
detail.[r_two_c_axle_index_config_id] AS r2c_axle_index,
steeraxle.value AS steer_axle,
driveaxle.value as drive_axle,
liftaxle.value AS lift_axle,
axles.value as axles,
location.code,
location.name AS depot_name,
location.aag_location_code,
location.depot_code,
location.address_line1 AS street_address,
location.city,
...
LEFT JOIN aggregated_odessa_dim_asset_vehicle_add_details_configs AS steeraxle ON steeraxle.bk_aggregated_dim_asset_vehicle_add_details_configs_id = addDetails.[steer_axle_id] AND steeraxle.current_flag = 1
LEFT JOIN aggregated_odessa_dim_asset_vehicle_add_details_configs AS driveaxle ON driveaxle.bk_aggregated_dim_asset_vehicle_add_details_configs_id = addDetails.[drive_axle_id] AND driveaxle.current_flag = 1
LEFT JOIN aggregated_odessa_dim_asset_vehicle_add_details_configs AS liftaxle ON liftaxle.bk_aggregated_dim_asset_vehicle_add_details_configs_id = addDetails.[lift_axle_id] AND liftaxle.current_flag = 1
LEFT JOIN aggregated_odessa_dim_asset_vehicle_add_details_configs AS axles ON axles.bk_aggregated_dim_asset_vehicle_add_details_configs_id = addDetails.[axles_id] AND axles.current_flag = 1
LEFT JOIN aggregated_odessa_dim_vehicle_detail detail ON detail.bk_aggregated_dim_vehicle_detail_id = Asset.bk_aggregated_dim_asset_id AND detail.current_flag = 1
LEFT JOIN aggregated_odessa_dim_asset_location AS assetlocation ON assetlocation.asset_id = Asset.bk_aggregated_dim_asset_id AND assetlocation.current_flag = 1 AND assetlocation.is_active = 1 AND assetlocation.is_current = 1
LEFT JOIN aggregated_odessa_dim_location AS location ON assetlocation.location_id = location.[bk_aggregated_dim_location_id] AND location.current_flag = 1 AND location.is_active = 1
...
SELECT
c.bk_aggregated_dim_contract_id AS contract_id,
c.[AAG Contract Number],
c.[Contract Sequence],
c.contract_commencement_date,
c.contract_maturity_date,
c.party_name,
c.account_legal_entity,
c.customer_term_in_months,
c.payment_frequency,
c.product_name,
...
a.bk_aggregated_dim_asset_id,
a.alias AS 'Alias',
a.registration_number AS 'Registration number',
a.asset_status AS status,
a.service_intervals_vehicle AS 'Service Intervals Vehicle',
a.service_intervals_tail_lift AS 'Service Intervals Tail Lift',
a.service_intervals_trailer AS 'Service Intervals Trailer',
a.service_intervals_fridgebox  AS 'Service Intervals Fridge Box',
a.road_fund_licence AS 'Road Fund Licence',
a.full_service_and_maintenance_chassis AS 'Full Service Maintenance Chassis',
a.full_service_and_maintenance_trailer AS 'Full Service Maintenance Trailer',
a.mot_preperationand_testing AS 'MOT Preperation Testing',
a.tail_lift_service_maintenance AS 'Tail Lift Service Maintenance',
a.fridge_service_maintenance as 'Fridge Service Maintenance',
a.crane_service_maintenance as 'Crane Service Maintenance',
a.twenty_four_hours_breakdown_cover AS '24 Hrs Breakdown Cover'
...
a.commencement_date AS 'Commencement Date',
a.customer_name AS 'Customer',
a.product AS Product,
a.cost_center,
a.payment_frequency AS 'Payment Frequency',
a.Term,
bdm.full_name AS 'Sales Person', 
a.line_of_business AS 'Line of Business',
a.created_date,
a.previous_schedule_number
FROM (
SELECT 
...
asset.*,
contracts.*,
commenced._date AS 'Origination Date'
...
LEFT JOIN commenced ON commenced.bk_aggregated_dim_contract_id = contracts.id AND rn = 1
WHERE previous_schedule_number IS NULL
...
a.term,
a.payment_method,
bdm.full_name AS sales_rep,
a.lease_contract_type,
a.base_rate AS COF,
a.margin,
a.total_yield AS interest_rate,
a.created_date
...
CAST(IRdetail.base_rate AS float) AS base_rate,
CAST(ROUND(IRdetail.spread, 2) AS float) AS margin,
IRdetail.interest_rate,
CAST(ROUND(flfd.total_yield, 2) AS float) AS total_yield

FROM aggregated_odessa_dim_contract AS c
JOIN aggregated_odessa_fact_lease_finance_detail AS flfd ON c.sk_aggregated_dim_contract = flfd.sk_aggregated_dim_contract
JOIN aggregated_odessa_dim_lease_finance AS lf ON flfd.sk_aggregated_dim_lease_finance = lf.sk_aggregated_dim_lease_finance AND lf.is_current = 1  and lf.current_flag = 1
    LEFT JOIN aggregated_odessa_dim_lease_interest_rate AS leaseIR ON leaseIR.lease_finance_detail_id = lf.bk_aggregated_dim_lease_finance_id AND leaseIR.current_flag = 1
...
LEFT JOIN aggregated_odessa_dim_line_of_business AS lob ON flfd.sk_aggregated_dim_line_of_business
 = lob.sk_aggregated_dim_line_of_business AND lob.current_flag = 1
WHERE c.current_flag = 1 AND COALESCE(booking_status, '') NOT IN ('Inactive', 'Pending', 'InstallingAssets')
...
CAST(ROUND(a.depreciation_value, 2) AS float) AS 'Equipment: Depreciation Value',
...
contract_id,
asset_id,
[Maintenance Cost Budget - Collection & Delivery],
[Third Party Fridge Charges],
[Maintenance Cost Budget - Crane],
[Third Party Ancillary Charges],
[Maintenance Cost Budget - RFL],
[Third Party Crane Charges],
[Maintenance Cost Budget - Overhead],
[Third Party Maintenance],
[Maintenance Cost Budget - General],
[Maintenance Cost Budget - Tyres],
[Maintenance Cost Budget - Tail Lift],
[Maintenance Cost Budget - Fridge],
[Maintenance Cost Budget - Relief Vehicle],
[Maintenance Cost Budget - Ancillary]
FROM (SELECT 
contract_id,
asset_id,
charges,
vat_amount,
charge_description
FROM vw_additional_charges_detail) as c
PIVOT (SUM(charges)
FOR charge_description IN ([Maintenance Cost Budget - Collection & Delivery],
[Third Party Fridge Charges],
[Maintenance Cost Budget - Crane],
[Third Party Ancillary Charges],
[Maintenance Cost Budget - RFL],
...
AS pvt
), asset_level_charges AS (

SELECT 
asset_id,
contract_id,
SUM([Maintenance Cost Budget - Collection & Delivery]) AS [Maintenance Cost Budget - Collection & Delivery],
SUM([Third Party Fridge Charges]) AS [Third Party Fridge Charges],
SUM([Maintenance Cost Budget - Crane]) AS  [Maintenance Cost Budget - Crane],
SUM([Third Party Ancillary Charges]) AS [Third Party Ancillary Charges],
SUM([Maintenance Cost Budget - RFL]) AS [Maintenance Cost Budget - RFL],
SUM([Third Party Crane Charges]) AS [Third Party Crane Charges],
SUM([Maintenance Cost Budget - Overhead]) AS [Maintenance Cost Budget - Overhead],
SUM([Third Party Maintenance]) AS [Third Party Maintenance],
SUM([Maintenance Cost Budget - General]) AS [Maintenance Cost Budget - General],
SUM([Maintenance Cost Budget - Tyres]) AS [Maintenance Cost Budget - Tyres],
SUM([Maintenance Cost Budget - Tail Lift]) AS [Maintenance Cost Budget - Tail Lift],
SUM([Maintenance Cost Budget - Fridge]) AS [Maintenance Cost Budget - Fridge],
SUM([Maintenance Cost Budget - Relief Vehicle]) AS [Maintenance Cost Budget - Relief Vehicle],
SUM([Maintenance Cost Budget - Ancillary]) AS [Maintenance Cost Budget - Ancillary]
 FROM pivoted_charges AS pc
GROUP BY 
asset_id,
contract_id
), total_charges AS (
...
c.payment_method,
c.sales_rep,
c.lease_contract_type,
c.COF,
c.margin,
c.interest_rate,
CAST(c.created_date AS date) AS contract_created_date,
...
t.total_charges
 ...
 FROM contracts AS c
JOIN lease_asset  AS la ON c.id = la.bk_aggregated_dim_contract_id AND la.rn = 1
